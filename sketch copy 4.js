...
let poses = [];
let connections;
let isFullScreen = false; // Variable to track full-screen mode
...
// Toggle full-screen mode when 'f' is pressed
function keyPressed() {
  if (key === "f" || key === "F") {
    isFullScreen = !isFullScreen;
    fullscreen(isFullScreen); // Toggle full-screen mode
    resizeCanvas(windowWidth, windowHeight); // Resize canvas for full screen
    video.size(windowWidth, windowHeight); // Adjust video size to match canvas
  }
}
